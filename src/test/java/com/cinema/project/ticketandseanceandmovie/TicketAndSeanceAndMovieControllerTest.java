...
public class TicketAndSeanceAndMovieControllerTest {
    ...
    public void getAllTicketsByUserId() {
        String modelAndView = "/ticket/mytickets";

        when(ticketAndSeanceAndMovieService.getTicketsForUser(user, Locale.CANADA)).thenReturn(ticketAndSeanceAndMovieList);
        when(model.addAttribute("tickets", ticketAndSeanceAndMovieList)).thenReturn(model);

        assertEquals(modelAndView, ticketAndSeanceAndMovieController.getAllTicketsByUserId(Locale.CANADA, user, model));
    }
}